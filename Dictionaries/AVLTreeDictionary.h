...
//methods
   ...
   void rotateRight(Node*& x) {
    ...
    void insert(const key_type& key, const mapped_type &value) { insert(root, key, value); }
    // insert elements after first one
    void insert(Node*& node,const key_type& key, const mapped_type &value)
    {
        ...
        else if (key == node->key) //replace
        {
            node->value = value;
            return;
        }

        if (key < node->key)  //insert further in the tree
        ...
        if (balance > 1 && key < node->left->key)           // left - left
            rotateRight(node);
        else if (balance < -1 && key > node->right->key)    // right - right
            rotateLeft(node);
        else if (balance > 1 && key > node->left->key)     // left - right
        {
    ...
    // insert element to dictionary - key and mapped type
    void insert(const value_type &key_value) { insert(key_value.first, key_value.second); }
